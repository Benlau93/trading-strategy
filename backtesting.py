...
class BackTesting:
    ...
    def backtesting(self, buy_and_hold: bool, verbose = True):
        ...
        for i in range(1,len(df)+1):
            if self.__position:
                ...
                if sell_signal[0]:
                    sell_value = (self.__position["NumShares"] * sell_signal[1]) - self.__fees
                    self.__capital += sell_value
                    closed_dict = {"SellDate":sell_signal[2],
                                    "SellPrice":sell_signal[1],
                                    "SellValue":sell_value}
                    self.__position.update(closed_dict)
                    # update trade
                    self.__trade += 1
                    # record transaction
                    ...
                    # verbose
                    if verbose:
                        print(f"Sold {num_shares} shares at ${round(sell_signal[1])} on {str(sell_signal[2])[:10]}")
            else:
                ...
                if buy_signal[0]:
                    ...
                    self.__position = {"BuyPrice":buy_signal[1],
                    ...
                    # verbose
                    if verbose:
                        print(f"Bought {num_shares} shares at ${round(buy_signal[1])} on {str(buy_signal[2])[:10]}")
        ...
        pl = closed["P/L"].sum()
        if self.__position:
            pl += ((df.sort_values("Date").tail(1)["Close"].iloc[0] * self.__position["NumShares"]) - self.__position["BuyValue"])
        pl = round(pl,2)
        pl_per = round(pl / self.__original_capital * 100,2)
        print(f"Trading Strategy: Total Profit of ${pl} ({pl_per}%) with {self.__trade} closed position(s)")
    ...
    def get_closed_position(cls):
        ...
        closed = pd.merge(buy_df, sell_df, on=["NumShares","Trade"])
    ...
    def export_result(cls, filepath):
        if len(cls.transaction) < 1:
            ...
            transaction_write = transaction_write[["Date","Action","Price","NumShares","Value"]].copy()
